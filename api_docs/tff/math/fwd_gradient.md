...
<meta itemprop="name" content="tff.math.fwd_gradient" />
...
```python
tff.math.fwd_gradient(
    func, x, input_gradients=None, use_gradient_tape=False
)
```
...
* <b>`func`</b>: A Python callable accepting one `Tensor` of shape of `x` and returning
